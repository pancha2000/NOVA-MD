 ...
 * Fix: isAdmin, isBotAdmin, isGroupAdmin now properly computed
 */
async function serialize(msg, conn) {
    ...
    // ✅ FIX: Body extraction with null safety
    ...
    // ✅ FIX: isAdmin / isBotAdmin - fetch group participants only for group messages
    m.isAdmin = false;
    m.isBotAdmin = false;
    ...
    if (m.isGroup) {
        try {
            const groupMeta = await conn.groupMetadata(m.from);
            ...
            m.isGroupAdmin = m.isAdmin; // alias
            m.groupMetadata = groupMeta;
            m.participants = participants;
        } catch (e) {
            // Group metadata fetch fail වුණොත් silently skip
            m.isAdmin = false;
            m.isBotAdmin = false;
        }
    }
